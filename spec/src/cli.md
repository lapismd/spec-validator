...
**Requirement.** The CLI MUST provide `validate`, `check`, `build`, `serve`, `first`, `search`, `index`, `list`, `init`, `doctor`, and `skill install` as first-class commands, its Deno entry point MUST use the Deno platform adapter, and its npm binary MUST remain available through package-manager symlinks using the explicit Node compatibility adapter.
